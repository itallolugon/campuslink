...
Completar o ciclo **CRUD** da aplicação, adicionando as operações de **edição** (Update) e **exclusão** (Delete) de eventos, que estavam ausentes nas sprints anteriores. Também foi corrigida a experiência do usuário na página "Minhas Inscrições", garantindo que controles administrativos não apareçam para o usuário final.

---

### 2. Funcionalidades Implementadas

#### 2.1 Edição de Evento (Update)

Um novo estado `eventoEditando` foi adicionado ao `App.jsx`. Ao clicar em "Editar" em um card, o evento é armazenado nesse estado e a navegação vai para a página de cadastro. O `FormEvento` detecta o prop `eventoInicial` e pré-preenche todos os campos:
...
O título e o botão de submit do formulário mudam conforme o modo:

| Modo | Título | Botão |
|---|---|---|
| Criação | "Cadastrar Novo Evento" | "Cadastrar Evento" |
| Edição | "Editar Evento" | "Salvar Alterações" |

#### 2.2 Exclusão de Evento (Delete)

A função `handleDeletar` remove o evento do array e cancela automaticamente qualquer inscrição vinculada a ele, mantendo a consistência dos dados:
...
Como ambos os estados (`eventos` e `inscritos`) são persistidos via `useEffect` no `localStorage`, a exclusão também é refletida no armazenamento local automaticamente.

#### 2.3 Botões de Admin Contextuais no EventCard

Os botões "Editar" e "Excluir" são renderizados condicionalmente: só aparecem quando os props `onEditar` e `onDeletar` são fornecidos. Na página "Minhas Inscrições", o `EventCard` é usado sem esses props, portanto o usuário não vê os controles administrativos:
...
| Arquivo | Mudança |
|---|---|
| `src/App.jsx` | Adição de `eventoEditando`, `handleEditar`, `handleDeletar`; refatoração de `handleCadastrar` |
| `src/components/FormEvento.jsx` | Suporte ao prop `eventoInicial` para modo de edição |
| `src/components/EventCard.jsx` | Botões de admin condicionais via props `onEditar`/`onDeletar` |
| `src/components/EventCard.module.css` | Estilos para `.acoesAdmin`, `.btnEditar`, `.btnDeletar` |
